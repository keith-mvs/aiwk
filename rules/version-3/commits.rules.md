...
| COM-012 | Formatting-only, dependency-only, configuration-only, and generated-output-only changes SHOULD be separate when separation improves review and does not break repository invariants. | Each separated commit remains buildable or the dependency is explicitly ordered. |
| COM-013 | Stage explicit paths or hunks. Avoid `git add -A`, `git add .`, and broad glob staging when unrelated work exists. | `git diff --staged --name-status` contains only task-owned paths. |
| COM-014 | Do not stage caches, logs, local overrides, secrets, temp files, crash dumps, or editor state. | Staged-path and secret review passes. |
